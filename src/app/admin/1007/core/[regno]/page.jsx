...
export default function Page({ params }) {
    ...
    const getData = async () => {
      try {
        const result = await axios.get(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/core/${regno}`
        );
  ...
  const handleSubmit = async (e) => {
    ...
    if (image) formData.append("coreimage", image);
  ...
  async function handleDelete() {
    if (window.confirm("Are you sure you want to delete this core member?")) {
      try {
        ...
        console.log(error);
        setNotificationMessage("Error deleting core member");
        setShowNotification(true);
        setColor("red");
      }
    }
